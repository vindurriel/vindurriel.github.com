(function($){
  ...
  var methods = {
    init: function(options) {
      ...
      $.getJSON(
        'http://tapirgo.com/api/1/search.json?callback=?&token=' + settings.token + '&query=' + settings.query, function(data){
          if(settings['complete']) { settings.complete() }
          $.each(data, function(key, val) {
            el.append('<div class="result"><h3><a href="' + val.link + '">' + val.title + '</a></h3><p>' + val.content.substring(0,100) + '...</p></div>');
          });
        }
      );
